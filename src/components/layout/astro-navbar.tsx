...
const luntsLogo = '/assets/lunts-logo.svg';
...
const Logo = styled.a<{ $isLunts?: boolean }>`
  display: flex;
  align-items: center;
...
const NavLink = styled.a<{ $isActive?: boolean; $isLunts?: boolean }>`
...
const DropdownLink = styled.a<{ $isLunts?: boolean }>`
  display: block;
  color: ${props => props.$isLunts ? 'white' : colors.bitsquid.contrast};
  text-decoration: none;
...
interface AstroNavbarProps {
  lang: string;
  currentPath: string;
}

const AstroNavbar: React.FC<AstroNavbarProps> = ({ lang, currentPath }) => {
  ...
  if (!isReady) {
    return (
      <NavbarContainer $isLunts={isLuntsPage} $isScrolled={false} style={{ visibility: 'hidden' }}>
        <NavContent>
          <div style={{ height: 70 }} />
        </NavContent>
      </NavbarContainer>
  ...
  const changeLanguage = (newLang: string) => {
    const currentSuffix = currentPath.replace(/^\/[a-z]{2}/, '');
    window.location.href = `/${newLang}${currentSuffix}`;
  };
  ...
  const isActiveRoute = (path: string) => {
    ...
    return normalizedPath === path || (path === '/' && normalizedPath === '');
  };
  ...
  return (
    <>
      ...
      <NavbarContainer $isLunts={isLuntsPage} $isScrolled={isScrolled}>
        <NavContent>
          <Logo href={`/${lang}`} $isLunts={isLuntsPage}>
            {isLuntsPage ? (
              <LogoImage 
                src={luntsLogo} 
                alt="Lunts" 
